...
contract Bank {
    ...
    function setAssetContract(address _t) public {
    ...
    event Debug(address indexed my_val);

     function newLoan(uint amount, uint dueDate, string memory mortgage, uint256 _tokenId) public {
        if(hasActiveLoan(msg.sender)) return;
        // transfer token to this contract
        // require(ac.getApproved(_tokenId) == msg.sender);
        // (bool success, bytes memory result) = _assetContractAddress.delegatecall(abi.encodeWithSignature("safeTransferFrom(address,address,uint256)", msg.sender, address(this), _tokenId));
        address fromAdd = msg.sender;
        // emit Debug(msg.sender);
        Asset(ac).safeTransferFrom(fromAdd, address(this), _tokenId);
        ...
        if(loanList[loanId].state == LoanState.ACCEPTING) {
            ...
            for(uint i = 0; i < loanList[loanId].proposalCount; i++) {
                ...
                if(proposalList[numI].state == ProposalState.ACCEPTED) {
                    // transfer mortgage
                    uint256 supply = Asset(ac).totalSupply();
                    string memory mort = loanList[loanId].mortgage;
                    uint256 tokenId = uint256(-1);
                    for(uint256 i=0;i<supply;i++){
                      string memory asset = Asset(ac).assets(i);
                      if(compareStrings(asset, mort) == true){
                        tokenId = i;
                        break;
                      }
                    }
                    Asset(ac).safeTransferFrom(address(this), msg.sender, tokenId);
                }
            } 
        }
    }
     ...
     function acceptProposal(uint proposeId) public
    {
     ...
     function getProposalAtPosFor(address lender, uint pos) public view returns(address, uint, ProposalState, uint, uint, uint, uint, string memory) {
...
// BORROWER ACTIONS AVAILABLE    
     ...
     function getLoanDetailsByAddressPosition(address borrower, uint pos) public view returns(LoanState, uint, uint, uint, uint,string memory) {
